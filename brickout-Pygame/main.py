...
lives = 3
current_level = 1
platform_speed = 5  # Define the platform speed


def reset_game():
    global ball_pos, ball_speed, platform_pos, platform_color, score, lives, current_level
    ball_pos = [WIDTH // 2, HEIGHT // 2]
...
# Define the fixed vertical position of the platform
PLATFORM_Y = HEIGHT - PLATFORM_HEIGHT - 30  # Adjust 30 to your desired distance from the ground
...
while game_running:
    ...
    # Ensure the platform stays within the screen boundaries horizontally
    platform_pos[0] = max(0, min(platform_pos[0], WIDTH - PLATFORM_WIDTH))

    # Set the platform's vertical position to the fixed value
    platform_pos[1] = PLATFORM_Y
